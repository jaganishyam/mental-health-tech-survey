"""
Predictive model: does an employee report having sought treatment for a
mental health condition, based on demographic and workplace-policy answers?

Trains two baseline classifiers (Logistic Regression, Random Forest) on the
cleaned survey data, picks the better one by ROC-AUC, and exposes everything
the Streamlit app needs to show model performance and serve live predictions.

Educational / portfolio use only — see the in-app disclaimer. This is not a
...
# Longest raw column names first, so "work_interfere" matches before a
# shorter false-positive prefix would.
_RAW_COLUMNS_BY_LENGTH = sorted(ALL_FEATURES, key=len, reverse=True)


def _raw_column(encoded_name: str) -> str:
    """Map a one-hot-encoded feature name (e.g. 'work_interfere_Not applicable')
    back to its underlying raw survey column ('work_interfere')."""
...
def feature_display_name(encoded_name: str) -> str:
    """Human-readable label for a raw or one-hot-encoded feature name, e.g.
    'work_interfere_Never' -> 'Work Interference Level'."""
    raw_col = _raw_column(encoded_name)
    return PRETTY_NAMES.get(raw_col, raw_col.replace("_", " ").title())

# A friendlier subset used to keep the live prediction form manageable.
FORM_FEATURES = [
    "Age", "Gender", "family_history", "work_interfere", "no_employees",
    "remote_work", "benefits", "care_options", "anonymity", "leave",
    "supervisor", "mental_health_consequence",
]
...
def _evaluate(pipeline, X_test, y_test, name):
    ...
    if hasattr(model, "feature_importances_"):
        ...
        # One-hot encoding splits each survey question into several columns
        # (one per answer choice) — sum their importances back together so
        # "Work Interference Level" appears once, not once per answer.
...
def train_models(df: pd.DataFrame):
    """Train Logistic Regression + Random Forest, return both results plus the best one."""
    data = df.copy()
...
def predict_probability(pipeline, form_values: dict, reference_row: pd.Series):
    """Build a single-row dataframe for prediction, filling any feature not on
    the simplified form with the dataset's most common (mode) value."""
    row = {}
    for feature in ALL_FEATURES:
        if feature in form_values:
            row[feature] = form_values[feature]
        else:
            row[feature] = reference_row[feature]
    for col in ["no_employees", "work_interfere", "leave"]:
        row[col] = str(row[col])
