...
| Streamlit App | `app.py`, `theme.py`, `model.py`, `utils.py` | A dark-themed, animated, recruiter-facing dashboard: detailed EDA with uncommon chart types, two trained ML models, and a live "try it yourself" prediction tool |
...
A dark, gradient-accented dashboard styled as a recruiter-facing analytics demo, with seven sections navigated from the sidebar:

- **🏠 Overview** — animated hero header, KPI cards (respondents, treatment rate, best model ROC-AUC, top predictor), project story, and key findings.
- **🔎 Explore & Segment** — inline filters (Gender, Age, Country, Company size) plus a mix of chart types: a **violin plot** (Age by treatment), a **sunburst** (Gender → Family History → Treatment), a **treemap** (Country → Company Size), and a **bubble chart** (Country: age vs. treatment rate vs. sample size).
...
- **🤖 Model Performance** — Logistic Regression and Random Forest classifiers trained on 18 cleaned features, compared by accuracy, ROC-AUC, precision, recall, and F1, with an ROC curve, confusion matrix, and feature-importance chart.
- **🔮 Try the Prediction Tool** — a live form; fill in your own answers and get the model's probability estimate on an animated gauge, with a clear educational-use disclaimer (this is not a diagnostic tool).
- **📈 Response Timeline** — an animated (play/pause) cumulative-responses chart built from the real survey timestamps (Aug 2014 – Feb 2016), plus a launch-window vs. long-tail comparison. *(Note: this is one survey wave with a long tail of late responses, not a separate 2016 survey — the page is explicit about that.)*
- **📌 Recommendations & About** — business recommendations, methodology/tech stack, and author info.

### ⚠️ About the prediction tool

The model predicts whether a respondent *reported having sought treatment*, based on patterns in a small, self-selected 2014 survey. It is a portfolio/educational demonstration of an ML workflow — **not a diagnostic tool**, and it should never be used to make real decisions about a real person. The in-app disclaimer says this explicitly.
...
├── theme.py                            # Dark theme CSS, color palette, chart styling helpers
├── model.py                            # ML pipeline: training, evaluation, live prediction
...
- **Plotly** — interactive, animated charts in the Streamlit app (violin, sunburst, treemap, bubble, radar, Sankey, parallel categories, gauge, animated timeline)
- **scikit-learn** — Logistic Regression and Random Forest models, evaluation metrics
- **Streamlit** — dashboard framework and hosting (Streamlit Community Cloud)
- **Custom CSS** (`theme.py`) — dark gradient theme, animated cards, styled nav
