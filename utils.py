"""
Shared data loading & cleaning utilities for the Mental Health in Tech Survey
Streamlit app. Mirrors the cleaning steps performed in the companion EDA
notebook (Mental_Health_Tech_Survey_EDA.ipynb) so both deliverables agree.
"""
import numpy as np
import pandas as pd
...
def load_data(path: str = DATA_PATH) -> pd.DataFrame:
    """Load survey.csv and apply the same cleaning steps as the EDA notebook."""
    df = pd.read_csv(path)
    df = df.copy()
...
def encode_for_correlation(df: pd.DataFrame) -> pd.DataFrame:
    """Numerically encode key columns for the correlation heatmap / pair plot."""
    cols = [
...
def get_mode_row(df: pd.DataFrame) -> pd.Series:
    """The most common value of every column — used to fill in features the
    live prediction form doesn't ask about."""
...
def workplace_support_index(df: pd.DataFrame) -> pd.DataFrame:
    """Score each of six workplace-support factors 0-100 (higher = more
    supportive), split by whether the respondent sought treatment. Used for
    the radar chart on the Correlations & Drivers page."""
    rows = []
